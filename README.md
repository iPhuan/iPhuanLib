...
IPHBaseModel为基础模型类，主要方便开发者可直接通过数据字典对对象进行初始化。与大家平常使用的主流的基础类不同的是，IPHBaseModel做了更大的改进和优化，IPHBaseModel不仅可以直接对字符串的属性进行映射初始化，还能对对象属性，数组属性进行映射，类似于响应链的模式可以使其符合要求的属性都能够得到映射和初始化。  
开发者需要继承于IPHBaseModel来进行使用，以下是对各个API做的相关说明：  

* **`- (NSDictionary *)attributeMapDictionary;`**   
> 对象属性映射字典。开发者在子类中重写该方法，并返回一个可用的字典。其中字典的Key为属性的名称，Value为方法`initWithDictionary:`中字典参数的Key，通过该映射方法，IPHBaseModel在初始化对象时才知道怎么从数据字典中取值。 
> 假设Json数据中root为一个酒店信息字典，包含了`name`，`hotle_id`，`recommended_room`，`room_list`等字段，其中`recommended_room`对应于一个推荐房间的字典，包含了房间的各个信息，room_list为一个数组，数组里面也是房间的字典 我们首先建立了一个叫`IPHRoom`的对象用来接收房间的信息，再建立一个`IPHHotle`的对象用来接收真个酒店的信息，`IPHRoom`和`IPHHotle`都继承于`IPHBaseModel`，于是我们可以在`IPHHotle`对象的`attributeMapDictionary`方法中这么写：  
...
> 通过代码示例，我们就很容易明白IPHBaseModel是怎么知道`recommendedRoom`和`rooms`属性所对象的数据类型了，通过该映射IPHBaseModel才能对二级数据进行处理，否则只能直接赋值没有经错处理的数据。  
> 同理，在`IPHRoom`中我们同样可以拥有对象和数组属性，当然前提是数据类型都为`IPHBaseModel`的子类对象，通过这种响应链方式的映射关系，一级一级的将数据初始化下去。  
> 当然该映射你也可以返回一个空字典，然后自己手动对`recommended_room`和`room_list`字段的数据进行特别处理处理。IPHBaseModel在初始化数据的时候调用了属性的Set方法，所以你只需要重写set方法，代码示例：  
...
> 一般建议通过`attributeTypesMapDictionary`映射的方式让IPHBaseModel自动帮你处理数据，除非你对数据的处理有特别要求，比如代码示例，需要单独再给数组中的`IPHRoom`对象设定`hotel`，此时可以进行手动处理。  
...
> 过滤的字段。当属性在初始化数据后，值为`filterStrings`数组里面的过滤字符串时，该属性会自动被设置为nil。目前过滤的字段如下：  
...
<br />
:warning:  
* IPHBaseModel会将Number类型的字典数据转化为字符串，你在使用IPHBaseModel创建类时，其对应的属性应当已字符串的形式创建。  
* 不仅仅是`initWithDictionary:`和`toDictionary`基于`attributeMapDictionary`映射来进行数据操作的。其在实现中`NSCoding`和`NSCopying`协议时也都是基于`attributeMapDictionary`来处理的，所以如果你希望你的数据能够正常处理，建议在`attributeMapDictionary`中添加其映射。
